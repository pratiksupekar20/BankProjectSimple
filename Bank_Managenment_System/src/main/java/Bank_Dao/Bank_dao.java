...
public class Bank_dao
{
	...
	public static String checkbalance(Bankbo bb)
	{
		...
		try
		{
			String sql="select Accbalance from signin where Cpawd=?";
	...
	public static boolean signin(String Acc_no, String Cpwd)
	{
		...
		try
		{
			String sql="select * from signin where Acc_no=? and Cpawd=?";
	...
	public static int insert(Bankbo bb)
	{
		int status=0;
			...
			try
			{
				String sql="insert into signin(Acc_no,Cname,Ccity,Accbalance,Cpawd) values(?,?,?,?,?)";
				PreparedStatement ps=con.prepareStatement(sql);
